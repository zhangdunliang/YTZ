...
public class OldCamera {
    ...
    public OldCamera(boolean isVertical) {
    ...
    //存储图片并显示
    public String onTakePhoto(byte[] data, ImageView imageView, String picName) {
        String filePath = null;
        Bitmap bitmap = null;
        BufferedOutputStream bos = null;
        try {
            bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
            File file = new File(YxPictureUtil.getAlbumDir(), picName);
    ...
    public RelativeLayout.LayoutParams getLayoutParam(Activity activity) {
        float scale = Math.min(screenWidth * 1.0f / cameraWidth, screenHeight * 1.0f / cameraHeight);
        int layout_width = (int) (scale * cameraWidth);
        int layout_height = (int) (scale * cameraHeight);

        if (mIsVertical) {
            scale = Math.min(screenWidth * 1.0f / cameraHeight, screenHeight * 1.0f / cameraWidth);
    ...
    public RelativeLayout.LayoutParams getParams(Camera camera) {
        ...
        // 注意Screen是否初始化
        Camera.Size bestPreviewSize = calBestPreviewSize(camPara, screenWidth, screenHeight);
        cameraWidth = bestPreviewSize.width;
        cameraHeight = bestPreviewSize.height;
