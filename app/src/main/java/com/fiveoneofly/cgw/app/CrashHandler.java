...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void init(Context context) {
        mContext = context;
        mDefaultHandler = Thread.getDefaultUncaughtExceptionHandler(); // 获取默认异常处理器
        Thread.setDefaultUncaughtExceptionHandler(this);// 将此类设为默认异常处理器
    }
...
//        if (BuildConfig.DEBUG)// debug 模式不处理
//            return false;
        ...
        new Thread() {// 在主线程中弹出提示
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(mContext, "程序小憩...", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
        }.start();
