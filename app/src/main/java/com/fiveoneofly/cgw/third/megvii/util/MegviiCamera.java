...
public class MegviiCamera {
    ...
    public Camera openCamera(Activity activity, int cameraId) {
        ...
        if (mCameraId == FRONT_CAMERA) {//前置-活体验证
            try {
                ...
                int height = ScreenUtil.getHeight(activity, true) - DensityUtil.dp2Px(activity, 45);
    ...
    public RelativeLayout.LayoutParams getLayoutParam(Context context) {
        if (mCameraId == FRONT_CAMERA) {//前置-活体验证
            ...
            float scale = Math.min(ScreenUtil.getWidth(context, true) * 1.0f / previewSize.height, (ScreenUtil.getHeight(context, true) - DensityUtil.dp2Px(context, 45)) * 1.0f / previewSize.width);//高度要减顶部导航栏高度
